...
def muestra_boxplot(df, vector_objetivo=""):
	n=1
	if vector_objetivo != "":
		df_temp = df.drop(columns=vector_objetivo)
	else:
		df_temp = df
	plt.figure(figsize=(15,15))
	for j in df_temp.columns:
		plt.subplot((len(list(df_temp.columns))/3)+2,3,n)
		ax=sns.boxplot(x=j,data=df_temp, orient='h')
		n+=1
	plt.tight_layout()
...
#def muestra_grilla_gam(modelo):  
#	#generamos la grilla para mostrar las dependencias parciales por atributo
#	x_grid = generate_X_grid(modelo)
#	attribute = X_train.columns
#	cols = 4; rows = np.ceil(len(attribute) / cols)
#
#	#mostramos las dependencias parciales por atributo
#	lista = {}
